...
SRC=("$REPO/tests/Tests/RepairTests.m" "$ROOT/App/ResourceResultRowView.m")
...
xcrun clang -fobjc-arc -g -O1 -fsanitize=address -mmacosx-version-min=13.0 \
  ...
  "${INCLUDES[@]}" "${SRC[@]}" -o "$OUT/RepairTests" >"$OUT/compile.log" 2>&1
ASAN_OPTIONS=detect_leaks=0 "$OUT/RepairTests" "$@"
