...
for f in StaticHTMLDiscoveryPageProbe RDManualVerification RDHybridPageProbe ProductionDiscoveryHTMLProvider ResourceURLGate ResourceDiscoveryCoordinator MultiPageResourceProbe MultiPageProbeResult SubpageLinkExtractor RDResourceModeFilter DetectedMedia RDQualityTier RDResourceDisplayMetadata WebProbe URLPolicy; do SRC+=("$ROOT/Features/ResourceDetector/$f.m"); done
