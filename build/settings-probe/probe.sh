...
SRC=("$REPO/build/settings-probe/SettingsProbe.m" "$ROOT/App/ResourceResultRowView.m")
...
xcrun clang -fobjc-arc -g -O1 -fsanitize=address -mmacosx-version-min=13.0 \
  ...
  "${INCLUDES[@]}" "${SRC[@]}" -o "$OUT/SettingsProbe" >"$OUT/compile.log" 2>&1
"$OUT/SettingsProbe" "$@"
printf 'PROBE DONE\n'
